...
def install_actuals_and_baselines(decoder, cl_args):
  ...
  decoder = dgen_actuals.AddAutoActualsToDecoder(decoder, decoder.table_names())
...
def main(argv):
  ...
  while remaining_args and remaining_args[0].startswith('--'):
    ...
    if index == -1:
      ...
      if (cl_name in cl_args.keys() and isinstance(cl_args[cl_name], list)):
  ...
  if output_filename.endswith('tests.cc'):
    ...
    dgen_test_output.generate_named_classes_h(
        decoder, decoder_name, _localize_filename(output_filename), f, cl_args)
  elif output_filename.endswith('_named_bases.h'):
    dgen_test_output.generate_named_bases_h(decoder, decoder_name,
    ...
    dgen_actuals.generate_actuals_base_h(decoder, decoder_name,
    ...
    dgen_baselines.generate_baselines_h(decoder, decoder_name,
                                        _localize_filename(output_filename), f,
                                        cl_args)
  elif output_filename.endswith('baselines.h'):
    dgen_baselines.generate_baselines_h(decoder, decoder_name,
    ...
    dgen_baselines.generate_baselines_cc(decoder, decoder_name,
                                         _localize_filename(output_filename), f,
                                         cl_args)
